...
import { EyeIcon, ImageIcon, Link2Icon, SendIcon } from '@lucide/vue';
...
const url = ref(props.featureDeal.url ?? '');
const status = ref(props.featureDeal.status);
const photoFiles = ref<File[]>([]);

const previewSrc = computed(() =>
    photoFiles.value[0]
        ? URL.createObjectURL(photoFiles.value[0])
        : `/storage/${props.featureDeal.photo}`,
);
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription
                            ...
                            cropping the focal point.</CardDescription
                        >
                    </CardHeader>
                    <CardContent>
                        <ImageDropzone
                            name="photo"
                            label="Replace photo"
                            hint="PNG, JPG or WEBP, up to 5MB. Leave empty to keep the current photo."
                            :error="errors.photo"
                            :processing="processing"
                            :initial-previews="[
                                `/storage/${featureDeal.photo}`,
                            ]"
            ...
            <div class="flex flex-col gap-6">
                <Card class="lg:sticky lg:top-6">
                    <CardHeader>
                        ...
                        <CardDescription
                            >How this tile will look on the
                            storefront.</CardDescription
                        >
                    </CardHeader>
                    <CardContent>
                        <div
