...
## content manager role added (2026-08-28)
Same scoped-role pattern as catalog manager / customer manager: full CRUD on `pages`, `blogs`, `blog categories` (user-picked subset, not all content-ish resources — help topics/faqs/testimonials/banners deliberately excluded) + view dashboard. No Users/Roles/Customers/orders/catalog access. Demo user content-manager@example.com / password in AdminSeeder. Covered by test_role_permission_seeder_grants_content_manager_full_control_over_content_resources in RolePermissionTest.php. Routes for pages/blogs/blog-categories already had permission middleware from the earlier route-level rollout — no routes/web.php change needed, seeder+demo-user only.
